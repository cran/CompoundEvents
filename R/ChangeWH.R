#'@title   Changes in compound humid-hot events
#'@description  Assess changes in compound humid-hot (WH) occurrences
...
#'@usage ChangeWH(pre,tem,threp,thret)
#'@return The absolute/relative change for each month and annual occurrence
#'@references Hao, Z., AghaKouchak, A., Phillips, T.J., 2013. Changes
#'in concurrent monthly precipitation and temperature extremes.
#' Environ. Res. Lett. 8(3), 034014.
#'@examples
...
ChangeWH<- function(pre,tem,threp,thret)
{
  ...
  for  (k in 1:12)
  {
    ...
    if (is.na(mdx[1])==TRUE)
    {
      ...
      res_mono[2:ny,k]<-CompoundEvents::GetWH(xd,yd,threp,thret)
    }

    else
      # if the first number is not NA, take the whole month to compute SPI
    {
      res_mono[,k]<-CompoundEvents::GetWH(mdx,mdy,threp,thret)
    }
  }
